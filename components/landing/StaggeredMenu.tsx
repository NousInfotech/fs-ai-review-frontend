...
export interface StaggeredMenuItem {
  ...
  ariaLabel: string;
  link: string;
  variant?: 'button-outline' | 'button-primary';
}
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  menuButtonColor = 'var(--landing-text-gray)',
  openMenuButtonColor = 'var(--landing-text-heading)',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {(() => {
            const raw = colors && colors.length ? colors.slice(0, 4) : ['var(--landing-primary-blue)', 'var(--landing-navbar-btn-bg)'];
            let arr = [...raw];
            if (arr.length >= 3) {
        ...
        >
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo">
            <Link href="/" onClick={closeMenu} className="flex items-center gap-2">
              ...
              />
              {title && (
                <span className="text-base font-normal text-(--landing-text-heading) tracking-tight">
                  {title}
                </span>
              )}
            </Link>
          ...
          <button
            ref={toggleBtnRef}
            className={`sm-toggle relative inline-flex items-center justify-center bg-transparent border-0 cursor-pointer pointer-events-auto transition-colors p-2 ${
              open ? 'text-black' : 'text-slate-600 hover:text-black'
            }`}
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    {it.variant ? (
                      <div className="sm-panel-item-btn">
                        <LandingButton
                          href={it.link}
                          variant={it.variant === 'button-primary' ? 'primary' : 'outline'}
                          className="w-full justify-center"
                          onClick={closeMenu}
                          icon={it.variant === 'button-primary' ? <ArrowRight className="w-5 h-5" /> : undefined}
                        >
                          {it.label}
                        </LandingButton>
                      </div>
                      ...
                      <Link
                        className="sm-panel-item relative font-medium text-[1.75rem] md:text-[3rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                        href={it.link}
                ...
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                  <span className="sm-panel-item relative font-medium text-[3rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
                    <span className="sm-panel-itemLabel inline-block origin-[50%_100%] will-change-transform">
                      No items
                    </span>
                  </span>
                  ...
                  {socialItems.map((s, i) => (
                    <li key={s.label + i} className="sm-socials-item">
                      <a
                        ...
                        className="sm-socials-link text-[1rem] md:text-[1.2rem] font-medium text-[#111] no-underline relative inline-block py-[2px] transition-[color,opacity] duration-300 ease-linear"
                      >
                        {s.label}
                      </a>
                    </li>
...
.sm-scope .sm-toggle:focus-visible { outline: 2px solid var(--landing-primary-blue); outline-offset: 4px; border-radius: 4px; }
...
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 2.5em; font-size: 14px; font-weight: 400; color: var(--sm-accent, #ff0000); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
